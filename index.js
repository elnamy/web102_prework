...
function addGamesToPage(games) {
  ...
  games.forEach((game) => {
    ...
    newGameCard.innerHTML = `<img class="game-img" src="${game.img}" alt="Game Image" />
...
// call the filterFundedOnly function to initially display only the funded games
filterFundedOnly();
...
const totalAmountRaised = GAMES_JSON.reduce((raised, game) => {
...
const totalGames = GAMES_JSON.reduce((numGames) => {
  return numGames + 1;
}, 0);
...
function showCollectedAmount() {
  let totalAmount = GAMES_JSON.reduce((total, game) => {
...
const unfundedGames = GAMES_JSON.reduce((unfundedGames, game) => {
  return game.pledged < game.goal ? unfundedGames + 1 : unfundedGames;
}, 0);

const fundedGames = GAMES_JSON.reduce((fundedGames, game) => {
  //return game.pledged >= game.goal ? fundedGames + 1 : fundedGames;
  return fundedGames + 1;
}, 0);

const fundedAmount = showCollectedAmount();

const remain = unfundedGames === 1 ? "remains" : "remain";
// create a string that explains the number of unfunded games using the ternary operator
const str = `A total of $${fundedAmount.toLocaleString(
  "en-US"
)} has been raised for ${fundedGames} ${fundedGames > 1 ? "games" : "game"}. 
Currently, ${unfundedGames} ${
  unfundedGames > 1 ? "games" : "game"
}  ${remain} unfunded. We need your help to fund this amazing games!`;
/*
`
...
topGameName.classList.add("info-text");
...
runnerUpName.classList.add("info-text");
